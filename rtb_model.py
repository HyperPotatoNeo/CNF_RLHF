...
class RTBModel(nn.Module):
    def __init__(self, prompt, reward_prompt, diffusion_steps=100, beta_start=1.0, beta_end=10.0):
        super().__init__()
        self.device = device
    ...
    def log_reward(self, x, return_img=False):
        with torch.no_grad():
            ...
            log_r = torch.tensor(self.reward_model.score(self.reward_prompt, img)).cuda()
        if return_img:
            return log_r, img
        return log_r
        
    def batched_rtb(self, shape, learning_cutoff=.1, prior_sample=False, loss_batch_size=64):
    ...
    def finetune(self, shape, n_iters=100000, learning_rate=5e-5, clip=0.01, wandb_track=False, prior_sample=False, anneal=False, anneal_steps=15000, loss_batch_size=64):
        B, *D = shape
        param_list = [{'params': self.model.parameters()}]
        optimizer = torch.optim.Adam(param_list, lr=learning_rate)
        run_name = self.prompt + '_beta_start_' + str(self.beta_start) + '_beta_end_' + str(self.beta_end) + '_anneal_' + str(anneal) + '_prior_sample_' + str(prior_sample)
        ...
        for it in range(n_iters):
            ...
            optimizer.zero_grad()
            loss, logr = self.batched_rtb(shape=shape, prior_sample=prior_traj, loss_batch_size=loss_batch_size)
